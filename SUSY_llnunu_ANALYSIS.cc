...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      IdentifiedFinalState muelfs(etaIn(-2.5, 2.5) & (pT >= 0.5*GeV));
      ...
      _hist_mll = bookHisto1D("mll", 50, 0.0, 600);
    ...
    void finalize() {
      ...
      normalize(_hist_phi_jet);
      normalize(_hist_eta_jet);
      normalize(_hist_pt_jet);
      normalize(_hist_n_l);
      normalize(_hist_phi_l);
      normalize(_hist_eta_l);
      normalize(_hist_pt_l, crossSection()/sumOfWeights());
