...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      _hist_deltaPhi_l = bookHisto1D("deltaPhi_l", 50, 0, PI);
      _hist_deltaPhi_l_cross = bookHisto1D("deltaPhi_l_cross", 50, 0, PI);

      _hist_deltaPhi_bjet = bookHisto1D("deltaPhi_bjet", 50, 0, PI);
      _hist_deltaPhi_bjet_cross = bookHisto1D("deltaPhi_bjet_cross", 50, 0, PI);
    }
    ...
    void analyze(const Event& event) {
      ...
      if (deltaPhi(bjets[0].momentum(), bjets[1].momentum())>1.2) {
        nvetoed7++;
		vetoEvent;
	  }
      

      nvetoed=nvetoed1+nvetoed2+nvetoed3+nvetoed4+nvetoed5+nvetoed6;
	  
	  //met histo  
      _hist_met->fill(met.vectorEt().mod()/GeV, weight);

	  //deltaPhi histo
      _hist_deltaPhi_l -> fill(deltaPhi(leptons[0].momentum(), leptons[1].momentum()), weight);
      _hist_deltaPhi_l_cross -> fill(deltaPhi(leptons[0].momentum(), leptons[1].momentum()), weight);

      _hist_deltaPhi_bjet -> fill(deltaPhi(bjets[0].momentum(), bjets[1].momentum()), weight);
      _hist_deltaPhi_bjet_cross -> fill(deltaPhi(bjets[0].momentum(), bjets[1].momentum()), weight);      
	 ...
	 _hist_n_bjet->fill(nbjets, weight);
      foreach (const Jet& j, bjets) {
        const FourMomentum& pj = j.momentum();
    ...
    void finalize() {
      ...
      //normalize(_hist_deltaPhi_l);

      //normalize(_hist_deltaPhi_bjet);

      scale(_hist_deltaPhi_bjet_cross, crossSection()/sumOfWeights());
      scale(_hist_deltaPhi_l_cross, crossSection()/sumOfWeights());
  ...
  private:
    ...
    Histo1DPtr _hist_ht;
    Histo1DPtr _hist_deltaPhi_l, _hist_deltaPhi_l_cross;    
    Histo1DPtr _hist_deltaPhi_bjet, _hist_deltaPhi_bjet_cross;  
    int nvetoed1;
    int nvetoed2;
    int nvetoed3;
    int nvetoed4;
